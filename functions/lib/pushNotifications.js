...
exports.sendDailyReviewPushes = void 0;
...
const getFirebaseApp = () => (0, app_1.getApps)().length ? (0, app_1.getApps)()[0] : (0, app_1.initializeApp)();
...
const localDateInfo = (date, timeZone) => {
...
const weekdayId = (weekday) => WEEKDAY_IDS[weekday] || '';
...
const ownerIdFromSubscriptionPath = (path) => {
 ...
 */
exports.sendDailyReviewPushes = (0, scheduler_1.onSchedule)({
    ...
    for (const [userId, subscriptionDocs] of subscriptionsByUser) {
        ...
        const configuredDays = Array.isArray(settings.daysOfWeek)
            ? settings.daysOfWeek.filter((day) => typeof day === 'string')
            : [];
        ...
        const configuredTopics = settings.topics && typeof settings.topics === 'object'
            ? settings.topics
            : undefined;
        if (configuredTopics?.cadernoErros === false)
            continue;
        const schedules = [
        ...
        const deliveredSlots = settings.lastDeliveryDate === localTime.dateKey
            ...
            ? settings.deliveredSlots.filter((slot) => typeof slot === 'string')
            : [];
        const dueSchedule = schedules.find((schedule) => localTime.minutes >= minutesFor(schedule.time)
        ...
        const pendingCount = Array.isArray(errorItems)
            ? errorItems.filter((item) => item && typeof item === 'object' && item.status !== 'dominado').length
            : 0;
        if (pendingCount === 0)
            continue;
        ...
        const invalidSubscriptionRefs = [];
        for (const batch of chunksOf(tokenEntries, 500)) {
            const response = await (0, messaging_1.getMessaging)(firebaseApp).sendEachForMulticast({
            ...
            response.responses.forEach((result, index) => {
                ...
                if (result.error && INVALID_TOKEN_CODES.has(result.error.code)) {
                    ...
                    firebase_functions_1.logger.warn('Falha ao enviar push SuVeCA.', {
                        userId,
        ...
        if (successfulDeliveries > 0) {
            await settingsRef.set({
                ...
                lastDeliveredAt: firestore_1.FieldValue.serverTimestamp(),
                updatedAt: firestore_1.FieldValue.serverTimestamp(),
            }, { merge: true });
            deliveredToUsers += 1;
        }
    }
    firebase_functions_1.logger.info('Rotina de push diário concluída.', {
